...
* Fix any systematic problems in the region taggers in
  ``server/clic/region``
* Export the tagged regions to a ``*.regions.csv`` file using
  ``server/bin/region_export``, any regions edited here will override
  the tagger.
...
:ref:`Managing a production instance of CLiC`.
...
First, make sure that the ``corpora.bib`` file has an entry for your new
book. Without an entry here, CLiC will not know to add it to the corpus,
and thus it will not appear in the drop-down. Re-import the book once
this has been done.
